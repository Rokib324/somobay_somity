  ...
  return (
    <header className="h-16 bg-white border-b border-slate-200 sticky top-0 z-30 px-4 md:px-6 flex items-center justify-between shadow-sm">
      ...
      <div className="flex items-center gap-3">
        {/* Branch indicator */}
        {user && (
          <div className="hidden md:flex items-center gap-2 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200/80 px-3 py-1.5 rounded-lg text-xs font-medium text-slate-700 shadow-sm">
            <i className="fa-solid fa-code-branch text-blue-600"></i>
            <span className="text-[11px] text-slate-500 font-medium">Branch:</span>
            <span className="font-bold text-slate-900">{user.branch || 'Head Office (Dhaka)'}</span>
          </div>
        )}
        ...
        <Link
          href="/sms/history"
          title="Messages & SMS Communications"
        ...
        {/* Dedicated Notification Bell */}
        <button
          title="System Notifications & Alerts"
          onClick={() => alert('All system alerts are up to date.')}
          className="relative p-2 text-slate-500 hover:text-slate-800 hover:bg-slate-100 rounded-lg transition-colors"
        >
          ...
          {showUserMenu && (
            <>
              ...
              <div className="absolute right-0 top-full mt-2 w-64 bg-white rounded-2xl shadow-xl border border-slate-200 z-20 overflow-hidden">
                ...
                {/* Menu items: My Profile, Account Settings, Security Settings, and Logout */}
                <div className="p-2 space-y-0.5">
                  <Link href="/settings" onClick={() => setShowUserMenu(false)}
                    ...
                    <i className="fa-solid fa-user w-4 text-center text-blue-600"></i>
                    My Profile
                  </Link>
                  <Link href="/settings" onClick={() => setShowUserMenu(false)}
                    className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs font-medium text-slate-700 hover:bg-slate-50 transition-colors">
                    <i className="fa-solid fa-sliders w-4 text-center text-slate-500"></i>
                    Account Settings
                  </Link>
                  <Link href="/settings/users" onClick={() => setShowUserMenu(false)}
                    className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs font-medium text-slate-700 hover:bg-slate-50 transition-colors">
                    <i className="fa-solid fa-shield-halved w-4 text-center text-emerald-600"></i>
                    Security Settings
                  </Link>
                </div>
